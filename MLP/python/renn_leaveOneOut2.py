...
def predict(verbose=True,k_max=200,E_stop=1e-3):
	...
	for m in range(0,leave_loop):
		...
		for k in range(0,test_loop):
			...
			y = np.empty((1,combinedData[3][k]-1))
			y0 = np.empty((1,7))
