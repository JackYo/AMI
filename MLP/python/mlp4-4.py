...
def predict2(verbose=True,k_max=200,E_stop=1e-3):
	...
	for k in range(0,test_loop):
		...
		Xpredict = np.empty((2,1))
		#Xpredict_ap = np.empty((3,1))
		Xpredict = combinedData[0][k][predictMin].reshape(2,1)
		...
		# use logistic
		mlp = pyrenn.CreateNN([2,4,4,1],dIn=[0,1,2,3],dIntern=[],dOut=[1,2,3])
	...
	with open(output_file_name, 'w') as csvfile:
		...
		csvfile.write('Test prediction MAE is:  {0}\n'.format(mae))
...
combinedData = readcsv('../data/rawData2.csv','../data/answerData.csv')
#combinedData = load_boston()
num = int(sys.argv[3])
output_file_name = "../output/output4-" + str(num) + ".csv"
#predict_complete()
